...
def apply_bias(values, bias, beta, legal_actions):
    biased_values = values.copy()
    bias_arr = np.array(bias)
    for a in legal_actions:
        biased_values[a] += beta * bias_arr[a]
    return biased_values


class StyledNFSPAgent(NFSPAgent):
    ...
    def step(self, state):
        '''
        From dqn_agent.py and nfsp_agent.py.
        '''
        obs = state['obs']
        legal_actions = list(state['legal_actions'].keys())
        
        if self._mode == 'best_response':
            q_values = self._rl_agent.predict(state)
            biased_q = apply_bias(q_values, self.style_bias, self.style_beta, legal_actions)
            
            epsilon = self._rl_agent.epsilons[min(self._rl_agent.total_t, self._rl_agent.epsilon_decay_steps-1)]
            probs = np.ones(len(legal_actions), dtype=float) * epsilon / len(legal_actions)
            best_action_idx = legal_actions.index(np.argmax(biased_q))
            ...
            probs = self._act_with_bias(obs, legal_actions)
            probs = remove_illegal(probs, legal_actions)
            action = np.random.choice(len(probs), p=probs)
    ...
    def eval_step(self, state):
        ...
        if self.evaluate_with == 'best_response':
            ...
            legal_actions = list(state['legal_actions'].keys())
            biased_q = apply_bias(q_values, self.style_bias, self.style_beta, legal_actions)
            action = np.argmax(biased_q)
            info = {}
            info['values'] = {state['raw_legal_actions'][i]: float(biased_q[list(state['legal_actions'].keys())[i]]) for i in range(len(state['legal_actions']))}
        elif self.evaluate_with == 'average_policy':
            obs = state['obs']
            legal_actions = list(state['legal_actions'].keys())
            probs = self._act_with_bias(obs, legal_actions)
    ...
    def _act_with_bias(self, info_state, legal_actions):
        info_state_t = np.expand_dims(info_state, axis=0)
        info_state_t = torch.from_numpy(info_state_t).float().to(self.device)
        
        with torch.no_grad():
            logits = self.policy_network.mlp(info_state_t).cpu().numpy()[0]
        
        biased_logits = apply_bias(logits, self.style_bias, self.style_beta, legal_actions)
        
        masked_logits = np.full(self._num_actions, -1e9)
        masked_logits[legal_actions] = biased_logits[legal_actions]
    ...
    def checkpoint_attributes(self):
        ...
        attrs = super().checkpoint_attributes()
        attrs['agent_type'] = 'StyledNFSPAgent'
        attrs['style_bias'] = self.style_bias
        attrs['style_beta'] = self.style_beta
        attrs['style_name'] = self.style_name
        return attrs
    
    @classmethod
    def from_checkpoint(cls, checkpoint):
        '''Restore from checkpoint, compatible with both NFSPAgent and StyledNFSPAgent'''
        print("\nINFO - Restoring StyledNFSPAgent from checkpoint...")
        
        # Check if this is a styled agent checkpoint
        style_bias = checkpoint.get('style_bias', [0.0, 0.0, 0.0, 0.0])
        style_beta = checkpoint.get('style_beta', 1.0)
        style_name = checkpoint.get('style_name', 'default')
    ...
    @classmethod
    def from_style_config(cls, config_path, style_name, **kwargs):
